...
from cart.schemas import CartResponse, CartSummaryResponse
...
def _summarize(items: list[dict]) -> tuple[float, int]:
    ...
    return round(float(total), 2), len(items)
...
async def add_to_cart(
    ...
    # movie via a paid OrderItem, per the "no repeat purchases" rule.
