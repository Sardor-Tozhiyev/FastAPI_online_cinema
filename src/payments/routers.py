...
from src.accounts.notifications import send_order_confirmation_email
...
from src.payments.models import Payment, PaymentItem, PaymentStatusEnum
from src.payments.schemas import CheckoutSessionResponse, PaymentResponse


logger = logging.getLogger("online_cinema_payments")

router = APIRouter(prefix="/api/v1/payments", tags=["payments"])


async def _payment_payload(
        db: AsyncSession,
        payment: Payment,
) -> dict:
    ...
    return {
        ...
        "items": [
            {
                "movies_id": item.order_item.movie.id,
                "name": item.order_item.movie.name,
                "price_at-payment": item.price_at_payment,
            }
            for item in items
        ]
    }
...
async def _handle_checkout_completed(
    ...
    payment = Payment(
        ...
        external_payment_id=session.get("client_intent"),
    )
    db.add(payment)
    await db.flush()
...
@router.get(
    ...
    summary="Get payment detail (owner or moderator)",
)
...
async def refund_payment(
    ...
    if payment.external_payment_id:
        try:
            ...
            raise HTTPException(
                status.HTTP_502_BAD_GATEWAY,
                detail="We couldn't reach the payment provider to refund this charge.",
            ) from exc
