...
async def test_login_success_returns_token_pair(
    ...
    await _register_and_activate(
        client, db_session, "login@example.com", strong_password
    )
...
async def test_login_wrong_password_returns_401(
    ...
    await _register_and_activate(
        client, db_session, "wrongpass@example.com", strong_password
    )
    response = await client.post(
        "/api/v1/accounts/login",
...
async def test_me_requires_valid_access_token(
    ...
    await _register_and_activate(
        client, db_session, "me@example.com", strong_password
    )
...
async def test_refresh_token_issues_new_access_token(
    ...
    await _register_and_activate(
        client, db_session, "refresh@example.com", strong_password
    )
    login_response = await client.post(
        "/api/v1/accounts/login",
...
async def test_logout_revokes_refresh_token(
    ...
    await _register_and_activate(
        client, db_session, "logout@example.com", strong_password
    )
    login_response = await client.post(
        "/api/v1/accounts/login",
